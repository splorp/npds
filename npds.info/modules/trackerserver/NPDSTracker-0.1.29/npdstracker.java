...
//		- The server now checks the hosts passed as REGUP parameters to be correct (to resolve and to be not private).
		...
		try {	// Global try.
			...
			} else if (theCommand.equals("REGUP"))
			{
				...
				else if ((QueryRecord(hname) == -1))
				{
					...
					// Check that it's not private network address.
					...
					if (theHostAddressAsBytes.length == 4)
					{
						...
						// 172.16.0.0/12
						if ((theHostAddressAsBytes[0] == 172) && ((theHostAddressAsBytes[1] & 0xF0) == 16))
						{
							throw new TQueryException ( kPrivateHostStr );
						}
						// 192.168.0.0/16
						if ((theHostAddressAsBytes[0] == 192) && (theHostAddressAsBytes[0] == 168))
						{
							throw new TQueryException ( kPrivateHostStr );
						}
					}
			...
			else if (theCommand.equals("GET"))
			{
				...
				synchronized (mHostInfoVector)
				{
					for (index_i = 0; index_i < mHostInfoVector.size(); index_i++)
					{
						...
						tableStr += "<tr valign=\"top\" class=\"" + classStr + "\"><td><p align=\"center\"><b>"
									+ labelStr
									+ "</b></p></td><td><a href=\"http://"
									+ theInfo.mName
									+ "\">" + theInfo.mDesc + "</a></td>";
				...
				if (index_i == 0)
				{
					tableStr += "<tr valign=\"top\" class=\"empty\"><td colspan=\"3\"><p align=\"center\"><i>No Newtons have registered with this Tracker.</i></p></td></tr>\r\n";
				}
				tableStr += "</table>\r\n";	
